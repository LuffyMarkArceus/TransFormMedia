...
func TestDeleteMedia_DoesNotDeleteStorageWhenNotOwner(t *testing.T) {
	...
	err := svc.DeleteMedia(context.Background(), "media-1", "attacker-user")
	if err == nil {
...
func TestDeleteMedia_DeletesStorageWhenOwner(t *testing.T) {
	processed := "https://cdn.example.com/processed/x"
	repo := &stubRepo{
		mediaByID: map[string]*media.Media{
			"media-1": {
